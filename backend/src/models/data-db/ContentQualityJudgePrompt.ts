...
export interface ContentQualityJudgePromptAttributes {
  id: string
  name: string
...
type ContentQualityJudgePromptCreationAttributes = Optional<
  ContentQualityJudgePromptAttributes,
  'id' | 'is_active' | 'is_system' | 'is_default'
>

export class ContentQualityJudgePrompt
  extends Model<ContentQualityJudgePromptAttributes, ContentQualityJudgePromptCreationAttributes>
  implements ContentQualityJudgePromptAttributes
{
  declare id: string
  declare name: string
...
export function initContentQualityJudgePromptModel(sequelize: Sequelize): typeof ContentQualityJudgePrompt {
  ContentQualityJudgePrompt.init(
    {
      id:               { type: DataTypes.UUID, defaultValue: DataTypes.UUIDV4, primaryKey: true },
      name:             { type: DataTypes.STRING(255), allowNull: false },
      description:      { type: DataTypes.TEXT,        allowNull: true },
      system_prompt:    { type: DataTypes.TEXT,        allowNull: false },
      threshold:        { type: DataTypes.REAL,        allowNull: false, defaultValue: 0.7 },
    ...
    {
      sequelize,
      tableName:   'content_quality_judge_prompts',
      modelName:   'ContentQualityJudgePrompt',
      timestamps:  true,
      underscored: true,
    }
  )
  return ContentQualityJudgePrompt
}
