...
module.exports = {
  async up(queryInterface) {
    ...
    const [rows] = await queryInterface.sequelize.query(
      `SELECT id, api_key FROM ai_providers WHERE api_key IS NOT NULL`
    )
    for (const row of rows) {
      const encrypted = encryptValue(row.api_key, secret)
      await queryInterface.sequelize.query(
        `UPDATE ai_providers SET api_key = :enc WHERE id = :id`,
  ...
  async down(queryInterface) {
    ...
    for (const row of rows) {
      try {
        ...
        await queryInterface.sequelize.query(
          `UPDATE ai_providers SET api_key = :val WHERE id = :id`,
